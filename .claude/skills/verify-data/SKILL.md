...
**Under-collection checks (added 2026-07-25).** These exist because the `ml` area shipped in the live default selection while NeurIPS and ICLR had *zero* works harvested and ICML was 88% missing affiliations — and every pre-existing check passed, since the `known-strong` tripwire only asserts non-zero and Stanford/MIT/CMU/Berkeley/UCLA all had non-zero counts off the ~12% of ICML that resolved.
- **Zero-work venues** (Phase 1, critical). Any venue registered in `areas.json` with no cached works at all. The pre-existing missing-year check only fired for venues carrying an explicit `years` map, so a venue registered with nothing but `openalex_ids` produced no finding of any kind. On its first run this caught `ral` and `scirobotics` — both `status=verified` in the default-on robotics area, both never harvested.
- **Per-area affiliation gap** (Phase 2). Rolls the venue-year quality scan up by area. Venue-level coverage can look fine while an entire area is blind, because one bad venue's slots dilute across many good venue-years. Thresholds: 50/25/15%, one severity lower for `default_off` areas.
- **Under-collected areas** (Phase 3). Institution count per area against the *median* area, not a fixed floor, so it self-calibrates as the corpus grows. Flags below 30% of median, critical below 15%.

**Non-research contamination checks (added 2026-07-25).** Adjusted count is 1/N per author, so a single-author editorial or column earns a full 1.0 — 4x what an author gets on a 4-author paper. 66 single-author science-fiction columns put Texas A&M at #1 in Science Robotics at double the runner-up.
- **Solo-author share per venue** (Phase 1). Flags venues where >8% (medium) or >15% (high) of *credited, non-excluded* works are single-author. Counting excludes already-curated works, so the flag clears as you curate rather than nagging forever. Currently flags `nature` 21.1%, `science` 19.2%, `nat-phys` 18.0% — all in `default_on: false` flagship.
- **Non-research titles still credited** (Phase 1). Stdlib regex backstop for titles containing "editorial", "erratum", "obituary", "book review", etc. that aren't in `data/excluded-works.csv` yet.
- **Do not act on author count alone.** `tit` (8.2%) and `automatica` (6.2%) are theory venues where solo-authored research is normal. Run `.venv/bin/python pipeline/detect_editorials.py --venue <key>` for semantic scoring, review the proposals, and promote accepted rows into `data/excluded-works.csv` yourself. The detector proposes; it never excludes. Its own output mixes real editorials ("Five years of Science Robotics") with real research papers ("Persuasive robots should avoid authority"), so the review step is load-bearing.

**Still a blind spot — check this by hand:**
- **Silently dropped credit.** A run that writes correct data which `aggregate.py` then discards looks identical to a healthy run. This actually happened on 2026-07-25: a backfill wrote 49,554 works whose institution objects lacked the `type` field, so `aggregate.py:199` filtered every one of them out, and `verify.py` returned a byte-identical finding set. Always compare the *magnitude* of adjusted counts against the previous run, not just orderings — and use low-gap areas (`comms`, `inftheory`) as a control group, since they should barely move when a backfill or normalization lands.
